                ...
                <div class="d-flex w-100" style="justify-content: space-between">
                    <h3>
                        ...
                        <?php if (!empty($products)): ?>
                            <span class="content-color"> (<span class="total-count"><?= $totalItems ?? 0 ?></span> Ürün) </span>
                        <?php endif; ?>
                    </h3>
                    <?php if (!empty($products)): ?>
                        <a href="#" class="btn btn-sm btn-outline-danger clear-cart-btn" style="font-size: 13px !important;letter-spacing: 1px;">
                            Sepeti Boşalt
                        </a>
                    <?php endif; ?>
                </div>
            ...
            <div class="row gy-3 gx-0">
                <?php if (!empty($products)): ?>
                    <?php foreach ($products as $product): ?>
                        <div class="col-12 product-row" data-product-id="<?= $product['id'] ?>">
                            <div class="product-box vertical-product">
                                <a href="<?= base_url('product/' . $product['id']) ?>" class="product-img">
                                    <img src="<?= !empty($product['image']) ?
                                        ...
                                        'http://localhost/web_kafe_otomasyon/public/assets/images/placeholder.jpg' ?>"
                                         class="img-fluid" alt="<?= esc($product['title']) ?>">
                                </a>
                                <div class="product-content">
                                    <h6 class="content-color"><?= esc($product['category_title']) ?></h6>
                                    <a href="<?= base_url('product/' . $product['id']) ?>" class="product-top">
                                        <h5 class="title-color white-nowrap"><?= esc($product['title']) ?></h5>
                                    </a>

                                    <?php if (!empty($product['note'])): ?>
                                        <div class="product-note">
                                            <small class="text-muted"><small><i><?= esc($product['note']) ?></i></small></small>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="bottom-content cart-content">
                                        ...
                                        <div class="plus-minus">
                                            <span onclick="updateQuantity(<?= $product['id'] ?>, -1)"
                                                  style="height: 25px">
                                                <?= getLucideIcon("minus", "icon") ?>
                                            </span>
                                            <input type="number" class="quantity-input"
                                                   value="<?= $product['quantity'] ?? 1 ?>" min="1" max="50"
                                ...
                                <div class="close-icon" onclick="showRemoveModal(<?= $product['id'] ?>)">
                ...
                <div class="bill-box-content">
                    <div class="d-flex align-items-center justify-content-between">
                        <h5 class="fw-medium content-color">Toplam (<span
                                    class="total-count"><?= $totalItems ?? 0 ?></span> Ürün)</h5>
                        <h4 class="fw-medium theme-color total-price"><?= number_format($totalPrice ?? 0, 2) ?> ₺</h4>
                    ...
                    <div class="total-amount">
                        ...
                        <h4 class="fw-medium theme-color total-price"><?= number_format($totalPrice - $discount, 2) ?>
                            ₺</h4>
                    </div>
                </div>
            </div>
...
<?php endif; ?>
    ...
    <script>
        ...
        // Ürün silme modalını göster (TÜM SİLME İŞLEMLERİ BU MODAL ÜZERİNDEN YAPILACAK)
        ...
        function confirmRemoveItem() {
            ...
            fetch('<?= base_url('cart/remove') ?>', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        const productRow = document.querySelector(`.product-row[data-product-id="${currentProductId}"]`);
                        if (productRow) {
                            productRow.remove();
                        }

                       // var cleanPrice = parseFloat(data.totalPrice.replace(/,/g, ''));

                        // Tüm .total-price elementlerini seç ve her birini güncelle
                        document.querySelectorAll('.total-price').forEach(element => {
                            element.textContent = parseFloat(data.totalPrice).toFixed(2) + ' ₺';
                        });

                        updateCartCounters(data.cart_count);

                        // Ürünü DOM'dan kaldır
                        const productElement = document.querySelector(`[data-product-id="${currentProductId}"]`);
